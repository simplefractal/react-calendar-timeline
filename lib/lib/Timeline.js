...
function nostack(items, groupOrders, lineHeight, force, activitiesArray) {
  var i, iMax;
  var totalHeight = 0;
  ...
  groupedItems.forEach(function (group, index) {
    ...
    var activitiesHeight = activitiesArray ? activitiesArray[index] * 50 : 0;
...
var ReactCalendarTimeline = function (_Component) {
  ...
  _createClass(ReactCalendarTimeline, [{
    ...
    value: function stackItems(items, groups, canvasTimeStart, visibleTimeStart, visibleTimeEnd, width) {
      ...
      var _stackingMethod = stackingMethod(dimensionItems, groupOrders, lineHeight, headerHeight, groupActivitiesArray),
